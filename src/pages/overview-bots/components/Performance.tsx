...
const Performance: React.FC<IPerformanceProps> = ({
  ...
  return (
    <div className="flex flex-col items-center w-full xl:w-auto mt-4 xl:mt-0">
      <h2 className="font-semibold text-xs text-dark-300 text-center mt-4 mb-2 uppercase">
        Active Bots
      </h2>

      <div className="w-[19.25rem] h-[1.9375rem] mb-4">
        <Switcher
          keyQuery="perf"
          query={query}
          tabs={tabs}
      ...
      {!isEmpty && (
        <span className="flex items-center gap-x-4">
          {[
            ...
            <span
              key={idx}
              className="flex justify-center items-center w-[2.25rem] h-[2.25rem] bg-light-250 text-blue-400 rounded-full cursor-pointer transition-colors duration-300 hover:text-white hover:bg-light-300"
            >
              {icon}
            </span>
          ))}
